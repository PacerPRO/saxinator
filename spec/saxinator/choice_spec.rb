...
module Saxinator
  RSpec.describe Choice do
    ...
    subject(:state_machine) { RET_choice(RET_text('hi'), RET_text('hello'), RET_text('hey')) }

    context 'first choice is taken' do
      let(:string) { 'hi' }

      it { expect(state_machine.parse(string)).to eq('hi') }
    end

    context 'a later choice is taken' do
      let(:string) { 'hey' }

      it { expect(state_machine.parse(string)).to eq('hey') }
    end
